...
// LOGIN
const NYTIMES_LOGIN_URL = 'https://myaccount.nytimes.com/auth/login?response_type=cookie&redirect_uri=https%3A%2F%2Fwww.nytimes.com';
...
@Injectable()
export class NytimesService implements OnModuleInit, Realm, ArticleScraper, LoginFlow  {
  private page: Page;
    ...
    if (loggedIn === true) {
      activeSubscription = pageData.user.type === 'sub'
                        && pageData.user.subInfo.subscriptions
                            .find(sub => sub.status === 'ACTIVE') !== undefined;
    }
   ...
   */
   async login(): Promise<Reader> {
    ...
    await page.screenshot({ path: SCREENSHOT_DIR + 'nytimes_login' + EXTENSION_PNG });
    ...
    await page.screenshot({ path: SCREENSHOT_DIR + 'nytimes_waitForNav' + EXTENSION_PNG });
